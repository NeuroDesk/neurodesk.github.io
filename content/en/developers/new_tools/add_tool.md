...
19) When the pull request is merged by Neurodesk admins, it will trigger an action to build the singularity container, distribute it in all object storage locations and on CVMFS, and it will update the menus in the desktop image on the next daily build. The new menu entry will be available after 24 hours in neurodesktop:latest and will be announced publicly with the next release.
